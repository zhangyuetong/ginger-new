...
文本模式末尾显示「显示前 N 条，还有更多；用 --limit 调大」。
...
python cli.py word aa --limit 200
```
...
```
与前端「释义含词」一致：**整词 + 词界**匹配（`taar` 不会命中 `taaru`）。用来反查哪些词的释义用到了这个词。
...
```
合法词性（12 个占位词）：`sj. k. e. i. a. hh. bu. p. q. cj. r. y.`。写 `k` 或 `k.` 均可，非法值会报错并列出全部可选项。

### `abbr` —— 查看词性标记含义表
```bash
python cli.py abbr
```
直接读取数据库中词性占位词条的 `guess_zh`，按固定顺序输出，例如：
```text
sj. 名词
k. 动词
...
